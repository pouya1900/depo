            ...
            <div class="content_box">
                ...
                <table id="myTable" class="table table-striped">
                    ...
                    @foreach($checks as $check)
                        <tr>
                            <td>{{$check->user->car}}</td>
                            <td>{{$check->user->name}}</td>
                            <td>{{$check->amount}}</td>
                            <td>{{jdate(strtotime($check->date))->format("Y-m-d")}}</td>
                            <td>{{$check->number}}</td>
                            <td>{{$check->bank}}</td>
                            <td><a class="table_action_green" href="{{route('edit_check',$check->id)}}">ویرایش</a>
                                <a class="table_action_red" href="{{route('delete_check',$check->id)}}">حدف</a>
                            </td>
                        </tr>
                    @endforeach
                    </tbody>
